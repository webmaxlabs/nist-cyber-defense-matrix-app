...
export function ReportHeader({ project }: ReportHeaderProps) {
  return (
    <div className="text-center border-b border-white/10 pb-8 mb-8 print:border-gray-300">
      ...
      <p className="text-sm text-slate-500 mt-2 print:text-gray-400">Generated on {formatDate(new Date().toISOString())}</p>
      {project.industry && <p className="text-sm text-slate-500 print:text-gray-400">Industry: {project.industry}</p>}
    </div>
  )
}
