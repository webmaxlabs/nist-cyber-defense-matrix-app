    ...
    <section className="mb-8">
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm border-collapse">
          <thead>
            <tr>
              <th className="border border-white/10 p-2 bg-white/[0.02] text-left text-slate-300 print:bg-gray-50 print:border-gray-300 print:text-black">Asset / Function</th>
              {NIST_FUNCTIONS.map((fn) => (
                <th key={fn} className="border border-white/10 p-2 bg-cyan-500/15 text-cyan-400 text-center print:bg-blue-600 print:text-white print:border-gray-300">
                  {NIST_LABELS[fn]}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {ASSET_CLASSES.map((asset) => (
              <tr key={asset}>
                <td className="border border-white/10 p-2 font-medium bg-white/[0.02] text-slate-300 print:bg-gray-50 print:border-gray-300 print:text-black">{ASSET_LABELS[asset]}</td>
                {NIST_FUNCTIONS.map((fn) => {
                  ...
                  return (
                    <td key={fn} className="border border-white/10 p-2 text-center print:border-gray-300">
                      {maturity ? (
